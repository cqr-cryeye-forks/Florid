...
class Consumer(object):
    ...
    def run(self):
        ...
        tasks_one = []
        print("[*] Starting phase one modules")
        for __module_name in lib.common.MODULE_ONE_OBJ_DICT:
            print(f"[*] Starting module: {__module_name}")
        ...
        for _t in tasks_one:
            _t.join()
        print("[*] Phase one modules completed")
        ...
        while not lib.common.CHECKER_OBJ.get_producer_state() or lib.common.CHECKER_OBJ.get_queue_length() > 0:
            ...
            if lib.common.CHECKER_OBJ.get_queue_length() > 0:
                ...
                print(f"[*] URL to be checked: {url_to_be_checked}")
                for __module_name in lib.common.MODULE_OBJ_DICT:
                    ...
                    print(f"[*] Submitting URL to module: {__module_name}")
                    t = threading.Thread(target=lib.common.MODULE_OBJ_DICT[__module_name].run, args=(url_to_be_checked,))
                    t.setDaemon(True)
                    t.start()

        print("[*] Consumer run completed")
